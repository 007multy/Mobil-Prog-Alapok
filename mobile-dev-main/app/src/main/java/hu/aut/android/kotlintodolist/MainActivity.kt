...
import hu.aut.android.kotlintodolist.adapter.TodoAdapter
...
import hu.aut.android.kotlintodolist.touch.TodoTouchHelperCallback
 ...
 */
class MainActivity : AppCompatActivity(), TodoItemDialog.TodoItemHandler {
    companion object {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fab.setOnClickListener { view ->
            TodoItemDialog().show(supportFragmentManager, "TAG_ITEM")
        }
    ...
    private fun initRecyclerView() {
        val dbThread = Thread {
            ...
            val items = AppDatabase.getInstance(this).todoItemDao().findAllItems()

            runOnUiThread{
                adapter = TodoAdapter(this, items)
                recyclerShopping.adapter = adapter

                val callback = TodoTouchHelperCallback(adapter)
    ...
    fun showEditItemDialog(itemToEdit: TodoItem) {
        val editDialog = TodoItemDialog()
...
/*Új Shopping Item-kor beszúrjuk a DB-be a DAO segítségével*/
    override fun todoItemCreated(item: TodoItem) {
        val dbThread = Thread {
            val id = AppDatabase.getInstance(this@MainActivity).todoItemDao().insertItem(item)
...
/*Update-or módosítjuk a Shopping Item-et a DAO segítségével*/
    override fun todoItemUpdated(item: TodoItem) {
        adapter.updateItem(item)

        val dbThread = Thread {
            AppDatabase.getInstance(this@MainActivity).todoItemDao().updateItem(item)
